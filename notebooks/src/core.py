...
class DetectLandmarks:
    def __init__(self, aorta_nrrd_file, overwrite=True):
        ...
        # writer.SetFileName(str(working_dir/'shared'/str(input_file_name)))
        # writer.Execute(self.sitk_img)

    def get_metadata(self):
        if not os.path.isfile(files['image_data']) or (self.overwrite):
    ...
    def drawings(self):
        if not os.path.isfile(files['drawings']) or (self.overwrite):
    ...
    def get_indices(self):
        ...
        if not os.path.isfile(files['indices']) or (self.overwrite):
            coronaries_candidates = argrelextrema(np.array(circle_similarity), np.less, order=2)[0]
            for i in coronaries_candidates:
                if circle_similarity[i] < triangle_similarity[i]:
                    coronaries_idx = i



            # slope = Math.calc_avg_slope(aorta_areas, 10)
            # top_idx = np.argmax(slope)
            # bottom_idx = np.argmin(slope)
            
            _aorta_areas = aorta_areas.copy()
            _aorta_areas.reverse()
            _slope = Math.calc_avg_slope(_aorta_areas, 10)
            n_slides = len(_aorta_areas)
            top_idx = n_slides - np.argmax(_slope)
            
            triangle_similarity_reverse = triangle_similarity
            triangle_similarity_reverse.reverse()
            
            _aorta_areas_sliced = _aorta_areas[top_idx:]
            # bottom_idx = n_slides - np.argmin(np.absolute(_aorta_areas_sliced)) - top_idx
            
            bottom_idx = n_slides - np.argmax(triangle_similarity_reverse)
    ...
    def get_triangles(self):
        ...
        if not os.path.isfile(files['triangles_info']) or (self.overwrite):
    ...
    def bottom_landmarks(self):
        ...
        indices = self.get_indices()
        drawings = self.drawings()
        ellipses, ellipses_params, eccentricity_list, angle_list = Image.draw_ellipses(self.arr)
    ...
    def plot_landmarks(self):
        ...
        for c, coords in enumerate(bottom_landmarks):
            ...
            cv2.putText(bottom_slide, cusp, org=(x - 30, y + 40), fontFace=cv2.FONT_HERSHEY_SIMPLEX,
                        fontScale=1, color=2, thickness=2, lineType=cv2.LINE_AA)
        ax1.imshow(bottom_slide)
        ax1.set_title('bottom landmarks')
        ...
        for i, coords in enumerate(top_landmarks):
            ...
            cv2.putText(top_slide_copy, label, org=(x-30, y+40), fontFace=cv2.FONT_HERSHEY_SIMPLEX,
                        fontScale=1, color=2, thickness=2, lineType=cv2.LINE_AA)
        ax2.imshow(top_slide_copy)
        ax2.set_title('top_landmarks')
        plt.savefig(str(FileFolders.folders['landmark']['landmark_shared']/'landmarks.png'))
        # plt.show()
    ...
    def export(self):
        ...
        results = {
            '4': bottom_landmarks[0],
            '6': bottom_landmarks[1],
            '5': bottom_landmarks[2],
            '1': top_landmarks[0],
            '3': top_landmarks[1],
            '2': top_landmarks[2]
        }
