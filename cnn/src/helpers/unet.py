...
class UNet:
    ...
    def u_route(self, x):
        ...
        conv1 = self.conv_bn_relu(x, filter_factor * 8, 3, 3)
        conv1 = self.conv_bn_relu(conv1, filter_factor * 8, 3, 3)
        ...
        conv3 = self.conv_bn_relu(pool2, filter_factor * 32, 3, 3)
        conv3 = self.conv_bn_relu(conv3, filter_factor * 32, 3, 3)
        pool3 = MaxPooling2D(pool_size=(4, 4))(conv3)

        conv4 = self.conv_bn_relu(pool3, filter_factor * 64, 3, 3)
        conv4 = self.conv_bn_relu(conv4, filter_factor * 64, 3, 3)
        pool4 = MaxPooling2D(pool_size=(4, 4))(conv4)

        conv5 = self.conv_bn_relu(pool4, filter_factor * 128, 3, 3)
        conv5 = self.conv_bn_relu(conv5, filter_factor * 128, 3, 3)

        up6 = concatenate([UpSampling2D(size=(4, 4))(conv5), conv4], axis=-1)
        conv6 = self.conv_bn_relu(up6, filter_factor * 64, 3, 3)
        conv6 = self.conv_bn_relu(conv6, filter_factor * 64, 3, 3)

        up7 = concatenate([UpSampling2D(size=(4, 4))(conv6), conv3], axis=-1)
        conv7 = self.conv_bn_relu(up7, filter_factor * 32, 3, 3)
        conv7 = self.conv_bn_relu(conv7, filter_factor * 32, 3, 3)

        up8 = concatenate([UpSampling2D(size=(2, 2))(conv7), conv2], axis=-1)
        conv8 = self.conv_bn_relu(up8, filter_factor * 16, 3, 3)
        conv8 = self.conv_bn_relu(conv8, filter_factor * 16, 3, 3)

        up9 = concatenate([UpSampling2D(size=(2, 2))(conv8), conv1], axis=-1)
        conv9 = self.conv_bn_relu(up9, filter_factor * 8, 3, 3)
        conv9 = self.conv_bn_relu(conv9, filter_factor * 8, 3, 3)

        conv10 = Convolution2D(self.n_classes, (1, 1), activation=self.non_linearity)(conv9)
        return conv10
