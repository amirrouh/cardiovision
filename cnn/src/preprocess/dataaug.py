...
class AugmentImage():
    ...
    def augment_image(image, angles_range, offset_range, mask = [], nr_duplicates=1, fill_value=0, size=[], spacing=[], origin=None, direction = []):
        augmented_images = []
        if type(mask) != list:
            augmented_masks = []
        for i in range(nr_duplicates):
            ...
            augmented_images.append(AugmentImage.transform_image(image, [angle1, angle2, angle3], [offset_x, offset_y, offset_z], int(fill_value), size, spacing, origin, direction))
            if type(mask) != list:
