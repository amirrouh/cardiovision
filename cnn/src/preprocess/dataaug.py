...
class AugmentImage():
    ...
    def add_noise_gaussian(img):   
        ...
        # Full resolution
        noise_im1 = np.zeros((rows//4, cols//4, ch//4))
        noise_im1 = random_noise(noise_im1, mode='gaussian', var=(val*2)**2, clip=False)
        noise_im1 = resize(noise_im1, (rows, cols, ch))

        # Half resolution
        noise_im2 = np.zeros((rows//8, cols//8, ch//8))
        noise_im2 = random_noise(noise_im2, mode='gaussian', var=(val*8)**2, clip=False)  # Use val*2 (needs tuning...)
        noise_im2 = resize(noise_im2, (rows, cols, ch))  # Upscale to original image size

        # Quarter resolution
        noise_im3 = np.zeros((rows//12, cols//12, ch//12))
        noise_im3 = random_noise(noise_im3, mode='gaussian', var=(val*12)**2, clip=False)  # Use val*4 (needs tuning...)
